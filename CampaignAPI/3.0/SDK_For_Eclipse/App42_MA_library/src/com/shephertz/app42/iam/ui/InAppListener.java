...
import com.shephertz.app42.iam.utils.InAppType;
...
/**
 * The listener interface for receiving inApp events.
 * The class that is interested in processing a inApp
...
public class InAppListener implements App42InAppListener {
		...
		currentActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (inAppType == InAppType.Alert) {
					new SimpleDialog(currentActivity, false,
							inAppData);
				} else if (inAppType == InAppType.Confirmation) {
					new SimpleDialog(currentActivity, true,
							inAppData);
				} else if (inAppType == InAppType.Survey) {
					new SurveyDialog(currentActivity).showSurveyDialog(inAppData);
				} else if (inAppType == InAppType.Interstitial) {
					CustomDialogData customData = (CustomDialogData) inAppData;
	...
	/* (non-Javadoc)
	 * @see com.shephertz.app42.iam.App42InAppListener#onCouponRedeemedFailed(java.lang.String, org.json.JSONObject)
	 */
	@Override
	public void onCouponRedeemedFailed(String couponCode,
			JSONObject couponProperties) {
		// TODO Auto-generated method stub
		App42Log.debug("InApp Redeeme Failed"+couponCode+couponProperties.toString());
	}

	/* (non-Javadoc)
	 * @see com.shephertz.app42.iam.App42InAppListener#onCouponRedeemedSucceeded(java.lang.String, org.json.JSONObject)
	 */
	@Override
