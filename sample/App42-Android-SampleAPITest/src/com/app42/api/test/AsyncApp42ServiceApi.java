...
public class AsyncApp42ServiceApi {
	 ...
	 */
	public void getUser(final String name, final App42ServiceListener callBack) {
		final Handler callerThreadHandler = new Handler();
		new Thread() {
			@Override
			public void run() {
	...
	public void insertJSONDoc(final String dbName, final String collectionName,
			final String json, final App42StorageServiceListener callBack) {
		final Handler callerThreadHandler = new Handler();
		new Thread() {
			@Override
			public void run() {
	...
	public void updateDocByKeyValue(final String dbName,
			...
			final String newJsonDoc, final App42StorageServiceListener callBack) {
		final Handler callerThreadHandler = new Handler();
		new Thread() {
			@Override
			public void run() {
	 ...
	 */
	public void uploadPic(final String name,
			...
			public void run() {
				try {
					...
					callerThreadHandler.post(new Runnable() {
						@Override
						public void run() {
							callBack.onUploadPicSuccess(response);
						}
					});
				} catch (final App42Exception ex) {
					callerThreadHandler.post(new Runnable() {
	 ...
	 * This function Uploads File On App42 Cloud.
	 */
	public void getPics(final App42UploadServiceListener callBack) {
		final Handler callerThreadHandler = new Handler();
		new Thread() {
			@Override
			public void run() {
				try {
					final Upload response = uploadService.getAllFiles();
					callerThreadHandler.post(new Runnable() {
						@Override
						public void run() {
							callBack.onGetPicsSuccess(response);
						}
					});
				} catch (final App42Exception ex) {
					callerThreadHandler.post(new Runnable() {
	...
	{
		void onUploadPicSuccess(Upload response);

		void onUploadPicFailed(App42Exception ex);

		void onGetPicsSuccess(Upload response);

		void onGetPicsFailed(App42Exception ex);
	}
